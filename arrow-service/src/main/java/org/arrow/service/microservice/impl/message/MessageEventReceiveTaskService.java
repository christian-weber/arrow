 ...
 */
@EventService
public class MessageEventReceiveTaskService implements EventMessageService<MessageEventRequest> {
    ...
    private class EventMessageCallable implements Callable<Iterable<EventMessage>> {
        ...
        public Iterable<EventMessage> call() throws Exception {
            ...
            ReceiveTask task = taskRepository.findReceiveTask(messageRef, true);

            if (task == null) {
                return FutureUtil.iterableOf();
            }
            ProcessInstance pi = piStore.store(task, request.getVariables());
            return Arrays.asList((EventMessage) new StartEventMessage(task, pi));
        }
    }
