 ...
 */
@CompoundService
public class MessageEventCompoundService implements EventMessageService<MessageEventRequest> {

    @Autowired
    private ActorSystem system;
    @Autowired
    private MessageEventProcessInvoker service1;
    @Autowired
    private MessageEventProcessResumingService service2;
    @Autowired
    private MessageEventSubProcessInvoker service3;
    @Autowired
    private MessageEventReceiveTaskService service4;
    ...
    public Future<Iterable<EventMessage>> getEventMessages(MessageEventRequest request) {
        ...
        if (request.isStartProcess()) {
            // composing the service results
            Future<Iterable<EventMessage>> msg1 = fallback(service1.getEventMessages(request));
            Future<Iterable<EventMessage>> msg2 = fallback(service4.getEventMessages(request));

            Future<Iterable<Iterable<EventMessage>>> result;
            result = Futures.sequence(Arrays.asList(msg1, msg2), system.dispatcher());
        ...
        // composing the service results
        Future<Iterable<EventMessage>> msg1 = fallback(service1.getEventMessages(request));
        Future<Iterable<EventMessage>> msg2 = fallback(service2.getEventMessages(request));
        Future<Iterable<EventMessage>> msg3 = fallback(service3.getEventMessages(request));
        Future<Iterable<EventMessage>> msg4 = fallback(service4.getEventMessages(request));

        Future<Iterable<Iterable<EventMessage>>> result;
        result = Futures.sequence(Arrays.asList(msg1, msg2, msg3, msg4), system.dispatcher());
