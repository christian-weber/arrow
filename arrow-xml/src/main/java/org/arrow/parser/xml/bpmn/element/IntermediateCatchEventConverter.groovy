...
import org.arrow.model.event.intermediate.catching.impl.*
...
public class IntermediateCatchEventConverter implements Converter {
    ...
    private static AbstractIntermediateCatchEvent determineIntermediateCatchEvent(Set<EventDefinition> definitions) {
        // @formatter:off
        switch (definitions) {
            // Multiple boundary event
            case { it.size() > 1 }: return new MultipleIntermediateCatchEvent(eventDefinitions: definitions)
            // Signal boundary event
            case { it[0] instanceof SignalEventDefinition }: return new SignalIntermediateCatchEvent(signalEventDefinition: definitions[0])
            // Message boundary event
            case { it[0] instanceof MessageEventDefinition }: return new MessageIntermediateCatchEvent(messageEventDefinition: definitions[0])
            // Timer boundary event
            case { it[0] instanceof TimerEventDefinition }: return new TimerIntermediateCatchEvent(timerEventDefinition: definitions[0])
            // Conditional boundary event
            case { it[0] instanceof ConditionalEventDefinition }: return new ConditionalIntermediateCatchEvent(conditionalEventDefinition: definitions[0])
            // Error boundary event
            case { it[0] instanceof LinkEventDefinition }: return new LinkIntermediateCatchEvent(linkEventDefinition: definitions[0])
            // default
